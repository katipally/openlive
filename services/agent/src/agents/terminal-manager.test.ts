...
test("captures interleaved output and the exit code", async () => {
  const tm = mk();
  const { terminalId } = tm.create({ command: "node", args: ["-e", "process.stdout.write('out'); process.stderr.write('err'); process.exit(3)"] });
...
test("output cap keeps the tail and sets truncated", async () => {
  ...
  const { terminalId } = tm.create({
    command: "node",
    args: ["-e", "process.stdout.write('x'.repeat(5000) + 'TAIL')"],
...
test("kill terminates a hanging command but keeps the buffer readable; release forgets it", async () => {
  const tm = mk();
  const { terminalId } = tm.create({ command: "node", args: ["-e", "process.stdout.write('started'); setInterval(() => {}, 1000)"] });
...
test("live output streams as batched term_output events + a final term_exit", async () => {
  ...
  const tm = mk(events);
  const { terminalId } = tm.create({ command: "node", args: ["-e", "process.stdout.write('hello')"] });
  await tm.waitForExit(terminalId);
