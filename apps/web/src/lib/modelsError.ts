import { providerAddress } from "@openlive/harness/registry";
import type { ProviderInfo } from "@openlive/harness/types";
...
export function classifyModelsError(e: unknown, provider?: ProviderInfo): ModelsFailure {
  ...
  // A local server is reached at an address the person set, so that is what to check.
  if (provider?.keyless) return { status: 502, reason: "network", error: `Couldn’t reach ${provider.name} at ${providerAddress(provider)}. Is it running?` };
